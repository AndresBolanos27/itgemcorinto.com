<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Asignar Grupos a ') . $subject->nombre }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('subjects.update-groups', ['subject' => $subject->id]) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                                    ...
                                    @foreach($groups as $group)
                                        <div class="flex items-center">
                                            <input type="checkbox" 
                                                ...
                                                name="groups[]"
                                                value="{{ $group->id }}"
                                                {{ $subject->groups->contains($group->id) ? 'checked' : '' }}
